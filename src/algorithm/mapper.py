...
def pi_genome_map(_input, max_wraps=0):
    """ The genotype to phenotype mapping process. Map input via rules to
    output. Returns output and used_input. """
    ...
    while (wraps < max_wraps) and \
        ...
        # Expand a production
        #ADD PI SELECTION HERE USING MOD RULE

        #THEN ADD CODE TO EXPAND ITEM
        possible_nt_postions = [unexpanded_symbols.index(i) for i in unexpanded_symbols if i[0][1] == NT_SYMBOL]
        position_index = possible_nt_postions[_input[used_input % n_input] % len(possible_nt_postions)]
        current_item = unexpanded_symbols.remove(position_index)

        current_symbol, current_depth = current_item[0], current_item[1]
        if current_max_depth < current_depth:
            current_max_depth = current_depth

        #REMOVED AS NEED TO DO TRAVESAL TO GET PHENOTYPE AT END
        # Set output if it is a terminal
        #if current_symbol[1] != NT_SYMBOL:
        #    output.append(current_symbol[0])
        #else:
        ...
        # Select a production
        # TODO store the length of production choices to avoid len call?
        current_production = _input[used_input+1 % n_input] % \
                             len(production_choices)
        # Use an input
        used_input += 2
        # Derviation order is left to right(depth-first)
        ...
        #Add children into list where parent was removed :D
        unexpanded_symbols[position_index:position_index] = children
    ...
    output = "".join(unexpanded_symbols)

    if any([i[0][1] == NT_SYMBOL for i in unexpanded_symbols]):
        # Not completly expanded, invalid solution.
        return None, _input, None, nodes, True, current_max_depth, \
               used_input
